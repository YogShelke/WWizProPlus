...
/***********************************************************************************************
  ...
  Description    : This method is used for Drwaing the Text and Setting the Text Font Accordig to Language Selection
				   Draw menu text
...
void CMenuXP::DrawText(CDC *pDC, CRect rect, CString strText, BOOL bSelected, BOOL bDisabled, BOOL bBold)
{
	...
	else
	{
		...
		if(m_dwSelectedLangID ==1 )//1 for Hindi Support
		{
			pOldFont = pDC->SelectObject(&theApp.m_fontWWTextSubTitle);
		}
	}
...
LRESULT CMenuXP::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	...
	UINT iSelect = 0;
	for (int i=0; i < static_cast<int>(nFound); i++) {
...
CMenuXP *CMenuXP::FindSubMenuFromID(DWORD dwID)
{
	CMenuXP	*pSubMenu;
	CMenuXP	*pResult;
	for (UINT i = 0; i < static_cast<UINT>(GetMenuItemCount()); i++)
	{
		if (GetMenuItemID(i) == dwID)
				return this;
	}

	for (int i = 0; i < static_cast<int>(GetMenuItemCount()); i++)
	{
