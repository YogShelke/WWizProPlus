...
// ISpyAVTray.pch will be the pre-compiled header
	...
	try
	{
		...
		if(pRtktOutFile != NULL)
		{
			...
			if(isDateTime)
			{
				...
				CString csProcId;
				csProcId.Format(L"%d", dwProcID);
				CString szOutMessage;
				szOutMessage.Format(_T("[%s %s] [VIBOSYSMTRAY:%s] %s\r\n"), dbuffer, tbuffer, csProcId, static_cast<LPCTSTR>(szMessage));
...
bool AddErrorServerLog(LPTSTR lpszTaskID, LPTSTR lpszTaskTypeID, LPTSTR lpszMachineIP, PWCH Message, ...)
{
	...
	try
	{
		...
		AddLogEntry(L"### %s", szMessageBuffer, 0, true, SECONDLEVEL);

		ISPY_PIPE_DATA szPipeData = { 0 };
		memset(&szPipeData, 0, sizeof(szPipeData));
		szPipeData.iMessageInfo = SEND_ERROR_LOG;
		szPipeData.dwValue = TASK_FAILD;
		wcscpy_s(szPipeData.szFirstParam, lpszTaskID);
		wcscpy_s(szPipeData.szSecondParam, lpszMachineIP);
		wcscpy_s(szPipeData.szThirdParam, Message);
		
		CISpyCommunicator objCom(EPS_CLIENT_AGENT, true, 0x02);
		if (!objCom.SendData(&szPipeData, sizeof(ISPY_PIPE_DATA)))
		{
			AddLogEntry(L"### Failed to set data in AddErrorServerLog", 0, 0, true, SECONDLEVEL);
			return false;
		}

		Sleep(100);
		memset(&szPipeData, 0, sizeof(szPipeData));
		szPipeData.iMessageInfo = UPDATE_TASK_STATUS;
		szPipeData.dwValue = TASK_FAILD;
		wcscpy_s(szPipeData.szFirstParam, lpszTaskID);
		wcscpy_s(szPipeData.szSecondParam, lpszTaskTypeID);
		wcscpy_s(szPipeData.szThirdParam, lpszMachineIP);
...
bool AddTaskCompletedServerLog(LPTSTR lpszTaskID, LPTSTR lpszTaskTypeID, LPTSTR lpszMachineIP, PWCH Message, ...)
{
	...
	try
	{
		...
		memset(&szPipeData, 0, sizeof(szPipeData));
		szPipeData.iMessageInfo = UPDATE_TASK_STATUS;
		szPipeData.dwValue = TASK_COMPLETED;
		wcscpy_s(szPipeData.szFirstParam, lpszTaskID);
		wcscpy_s(szPipeData.szSecondParam, lpszTaskTypeID);
		wcscpy_s(szPipeData.szThirdParam, lpszMachineIP);
