...
bool CZipArchive::CloseNew(int iAfterException, bool bUpdateTimeStamp)
{
	// if after an exception - the archive may be closed, but the file may be opened
	if (IsClosed() && (!iAfterException || IsClosed(false)))
	{
                              ...
                              DWORD nBufSize)
{
	...
	else
	{
		...
		m_dwFileSize = pHeader->m_uUncomprSize; // added by lalit to calculate the percentage done.

		CZipFile f(szFile, CZipFile::modeWrite | 
			CZipFile::modeCreate | CZipFile::shareDenyWrite);		
		DWORD iRead;
		CZipAutoBuffer buf(nBufSize);
		m_dwBufferSize = nBufSize;  // added by lalit to calculate the percentage done.
		m_dwDone = 0;   // added by lalit to calculate the percentage done.
		for(;;)
		{
			...
			// issue ;- to break the loop in case of abort de- compression.
			// resolved by lalit kumawat 4-7-2015
			if (m_bIsAborted)
			{
				break;
			}
...
bool CZipArchive::AddNewFile(CZipAbstractFile& af, LPCTSTR lpszFileNameInZip, int iComprLevel,  int iSmartLevel,   unsigned long nBufSize)
{
...
bool CZipArchive::AddNewFile(CZipAddNewFileInfo& info)
{
	...
	if (bCheckForSmallFiles || bCheckForZeroSized || bNeedTempArchive)
	{
		...
		m_dwFileSize = uFileSize; // added by lalit to calculate percentage done.
	...
	DWORD iRead;
	int iAborted = 0;
	m_dwBufferSize = (DWORD)info.m_nBufSize; // added by lalit to calculate percentage done;
	m_dwDone = 0;
	do
	{
		...
		m_dwDone++;
		m_bIsproStart = true;
		if (m_bIsAborted) // to break the compression if stop called from UI
		{
			break;
		}
	}
