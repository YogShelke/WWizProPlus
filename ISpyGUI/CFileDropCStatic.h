...
typedef HRESULT (CALLBACK FAR * LPFN_DROP_FILES_CALLBACK)(CStatic*, const CString&, const UINT&);
//
...
class CFileDropCStatic : public CStatic
{
public:
	CFileDropCStatic(CListCtrl *m_List);
	CFileDropCStatic();
	virtual ~CFileDropCStatic();
	...
	BOOL SetDropMode(const CFileDropCStatic::DROPLISTMODE& dropMode);
...
public:
	...
	CFileDropCStatic& operator=(const CFileDropCStatic& x);
	CFileDropCStatic(const CFileDropCStatic& x);
	BOOL m_bMaxLimitReached;
