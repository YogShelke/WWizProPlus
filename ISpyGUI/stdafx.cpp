...
/**********************************************************************************************************                     
*  Function Name  :	ExtractTokenString                                                     
*  Description    :	Extact token string.
...
bool ExtractTokenString(CString csCombineString, CStringArray &csaStringsList)
{
	...
	CString csEachString = L"";
	csEachString = csCombineString.Tokenize(_T("#"), iPos);
	
	while(csEachString!=_T(""))
	{
		csaStringsList.Add(csEachString);
		csEachString = csCombineString.Tokenize(_T("#"),iPos);
	};

	if(csaStringsList.GetCount() > 0)
		return true;
...
DWORD GetLoggingLevel4mRegistry()
{
	...
	__try
	{
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("SOFTWARE\\Wardwiz Antivirus"), 0, KEY_QUERY_VALUE | KEY_WOW64_64KEY, &h_WRDWIZAV) != ERROR_SUCCESS)
		{
			h_WRDWIZAV = NULL;
			return 0;
		}
