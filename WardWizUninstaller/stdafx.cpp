...
// WardWizUninstaller.pch will be the pre-compiled header
// stdafx.obj will contain the pre-compiled type information

#include "stdafx.h"
#include "WardwizLangManager.h"

#define LOG_FILE					_T("VIBOUNINSTALL.LOG")
	...
	bool isDateTime, DWORD dwLogLevel)
{
	try
	{
		...
		if (csScanLogFullPath.GetLength() == 0)
		{
			...
			/*if(!GetTempPath(MAX_PATH, szModulePath))
			{
			MessageBox(NULL, L"Error in GetModulePath", L"Vibranium", MB_ICONERROR);
			return;
			}*/

			//ISSUE : Log for WardWizUninstaller is getting created at incorrect path.
			//NAME - Niranjan Deshak. - 31th March 2015
			csScanLogFullPath = szModulePath;
			csScanLogFullPath += L"\\Vibranium\\Log\\";
	...
	{
		AddLogEntry(L"### VibraniumUNINSTALLER : Error in AddLogEntry", 0, 0, true, SECONDLEVEL);
	}
}
