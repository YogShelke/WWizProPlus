...
OutputDir=.\Output
OutputBaseFilename=incompcheckx64	
...
var
GetAvProductx64Name,GetWWIZDriverFolder,Gettempfolder,ISRegKeyPathExist,GetFinal32AvRegInfo,GetFinal64AvRegInfo,TempAvName,FinalAntivirusName,FinalUninstallFirstParameter,FinalUninstallSecondParameter,GetUninstallString,GetAllUninstallAvList,GetUninstallStringSecondParam,GetUninstallStringFirstParam:String;
UninstallotherAvParam,GetUninstallFirstParam,GetUninstallStringFirstParamString:String;
GetFirstParamUninstallString,GetSecondParamUninstallString,GetAvHKLM32Reg,GetAvHKLM64Reg,ISRegKeyPath32Exist,ISRegKeyPath64Exist,FinalUninstallStringAv,Av_DisplayIcon,Av_DisplayName,Av_Publisher,Av_UninstallString:String;
...
var
  CheckBitdefenderAvHKLM64Reg:String;
...
begin
    ...
    CheckBitdefenderAvHKLM64Reg:='SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Bitdefender';
    if(ISRegKeyPath64Exist=CheckBitdefenderAvHKLM64Reg)then
    begin
    ...
    begin
        ...
        else if(FinalUninstallSecondParameter<>'') then
        begin
			...
			CheckBitdefenderAvHKLM64Reg:='SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\InstallWIX_{D891550B-ACFE-4797-B368-BCFC434BBEB1}';
            if(ISRegKeyPathExist=CheckBitdefenderAvHKLM64Reg)then
            begin 
            ...
            i := i + 1;
             ...
             begin
                   ...
                   begin
                        ...
                        begin
                            RegQueryStringValue(GetHKLM,ISRegKeyPath64Exist,'DisplayIcon', Av_DisplayIcon);
              ...
              begin
								if(Av_DisplayName='') then
                            begin
                                 GetFinal64AvRegInfo:=Av_DisplayIcon+' | '+Av_Publisher+' | '+GetFirstParamUninstallString+' | '+GetSecondParamUninstallString;
                            end
                            else
                                GetFinal64AvRegInfo:=Av_DisplayIcon+' | '+Av_DisplayName+' | '+GetFirstParamUninstallString+' | '+GetSecondParamUninstallString;
                end;
                                if(GetFinal64AvRegInfo='')then
                                 begin
                                      ExitProcess(0);
                                 end
                                 else
									GetUninstallAvInfo(GetFinal64AvRegInfo);
							 end;                        
end;
        End Else Begin
...
begin
  GetWWIZDriverFolder:=ExpandConstant('{sd}\')+ExpandConstant('{#WRDWIZPFPATH}')+'\WARDWIZ\DRIVERS';
	...
	begin
				GetAvProductx64Name:=('{#SetupSetting("OutputBaseFilename")}');
				RegisterAvInstallerWithDrivers(GetAvProductx64Name);
				...
				if((GetFinal64AvRegInfo='')and(GetFinal32AvRegInfo='')) then
	      begin
							UnloadDLL(ExpandConstant('{tmp}\WRDWIZSETUPDLL.DLL'));
							DeleteFile(ExpandConstant('{tmp}\WRDWIZSETUPDLL.DLL'));
							DelTree(ExpandConstant('{tmp}'), True, True, True);
							if FileExists(ExpandConstant('{tmp}\incompcheckx64.tmp')) then
							begin
										DelayDeleteFile(ExpandConstant('{tmp}\incompcheckx64.tmp'), 2);
									DelTree(ExpandConstant('{tmp}'), True, True, True);
							end;
