...
#define AppVer ReadIni("D:\AcutokWinDev_1.1\ACBinary.ini", "ProductVersion", "ProductVer", "2.0.0.1")
...
OutputDir=.\Output
OutputBaseFilename=VibroASPreSCNx64	
...
CM_14_WWSetupDllPREINSTSCN  =Initializing Pre-Installation Scan, Please wait.
english.CM_14_WWSetupDllPREINSTSCN  =Initializing Pre-Installation Scan, Please wait.
...
Source: ..\Release\Win32\Binaries\VBSETUPDLL.DLL; DestDir: {tmp};Flags:dontcopy
Source: ..\Release\x64\Binaries\VBHEUSCN.DLL; DestDir: {tmp};Flags:dontcopy 
Source: ..\Release\x64\Binaries\VBSCANDLL.DLL; DestDir: {tmp};Flags:dontcopy 
Source: ..\Release\x64\Binaries\VBPREINSTSCAN.EXE; DestDir: {tmp}; Flags:dontcopy
Source: ..\Release\x64\Binaries\VBREPAIRDLL.DLL; DestDir: {tmp};Flags:dontcopy 
Source: ..\Release\x64\Binaries\VBRKSCN.DLL; DestDir: {tmp};Flags:dontcopy 
Source: ..\Release\x64\Binaries\VBRESOURCE.DLL; DestDir: {tmp};Flags:dontcopy 
Source: ..\Release\x64\Binaries\VBHASH.DLL; DestDir: {tmp}; Flags:dontcopy
Source: SETTINGS\ESSENTIALPLUSSETTING\PRODUCTSETTINGS.INI; DestDir: {tmp};  Flags:dontcopy onlyifdoesntexist
...
;Source: RequiredFiles\VIBRANIUMSECURITYNEWS\VIBRANIUMSECURITYNEWS.TXT; DestDir: {tmp}; Flags:dontcopy ignoreversion 
Source: ..\Release\x64\Binaries\VBMD5SCN.DLL; DestDir: {tmp};Flags:dontcopy
Source: RequiredFiles\SCITERDLL\64\SCITER64.DLL; DestDir: {tmp}; Flags:dontcopy
Source: ..\Release\x64\Binaries\VBPOLYSCN.DLL; DestDir: {tmp};Flags:dontcopy
Source: RequiredFiles\BDS\x64\*; DestDir: {tmp}; Flags: ignoreversion recursesubdirs createallsubdirs overwritereadonly
...
;Source: RequiredFiles\SVDB\*; DestDir: {tmp}\SVDB; Flags:dontcopy ignoreversion recursesubdirs createallsubdirs  overwritereadonly
...
begin
 PreInstallScanForm.Show;
     ...
     if(GetSelectedLanguage='german') then
		...
		ExtractTemporaryFiles('{tmp}\VBDB\*.*');
		PreInstallScanForm.Refresh;
		ExtractTemporaryFiles('{tmp}\PLUGINS\*.*');
		CreateDir(ExpandConstant('{tmp}\VBSETTINGS'));
		...
		GetSettingsFolder:=ExpandConstant('{tmp}\VBSETTINGS\');
...
begin
    ...
    ForceDirectories(ExpandConstant('{sd}\')+ExpandConstant('{#WRDWIZPFPATH}')+'\VIBRANIUM\VBDB');
		Exec(ExpandConstant('{cmd}'),Format('/C xcopy  "%s" "%s" /O /X /E /H /K /Y ', [ExpandConstant('{tmp}\VBDB\*.*'),GetWWizDBPath]),'', SW_HIDE, ewWaitUntilTerminated, ResultCode);
