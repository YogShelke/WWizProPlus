...
OutputDir=.\Output
OutputBaseFilename=WardWizEssentialPatchGx86
...
CMWWPatchValidForEss =This patch is applicable to WardWiz Essential only, Please install appropriate patch
english.CMWWPatchValidForEss =This patch is applicable to WardWiz Essential only, Please install appropriate patch
german.CMWWPatchValidForEss =Dieser Patch ist für WardWiz Essential nur, Bitte installieren Sie entsprechende Patch
...
Source: SETTINGS\ESSENTIALSETTING\PRODUCTSETTINGS.INI; DestDir: {app}\WRDSETTINGS; Flags: ignoreversion overwritereadonly onlyifdoesntexist
...
Root: HKLM; Subkey: SOFTWARE\Wardwiz; ValueType: dword; ValueName: dwProductID; ValueData: 1; Flags: uninsdeletekey
...
begin
  ...
  if (CurPage = 1) then
   ...
   begin
    ...
    RegKey := Format('%s', ['SOFTWARE\Wardwiz']);
    RegKey := ExpandConstant(RegKey);
    ...
    RegQueryDWordValue(HKEY_LOCAL_MACHINE, RegKey,'dwProductID', ProductID);    //RegQueryDWordValue
    if not ( ProductID = 1) then
    begin
      MsgBox(ExpandConstant('{cm:CMWWPatchValidForEss}'), mbError, MB_OK);
      ExitProcess(0);
    end;
	Result := CheckForPreviousVersion('ESSENTIAL',CurrentVersion,true);
