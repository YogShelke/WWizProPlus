...
OutputDir=.\Output
OutputBaseFilename=WardWizBasicPatchGx86
...
CMWWPatchValidForBasic =This patch is applicable to WardWiz Basic only, Please install appropriate patch
english.CMWWPatchValidForBasic =This patch is applicable to WardWiz Basic only, Please install appropriate patch
german.CMWWPatchValidForBasic =Dieser Patch ist für WardWiz Basic nur, Bitte installieren Sie entsprechende Patch
...
Source: SETTINGS\BASICSETTING\PRODUCTSETTINGS.INI; DestDir: {app}\WRDSETTINGS; Flags: ignoreversion overwritereadonly onlyifdoesntexist
...
Source: RequiredFiles\WRDWIZHELP\WRDWIZBASIC\WRDWIZBASIC.CHM; DestDir: {app}; Flags: ignoreversion overwritereadonly
Source: RequiredFiles\WRDWIZHELP\WRDWIZBASIC\WRDWIZBASICGERMAN.CHM; DestDir: {app}; Flags: ignoreversion overwritereadonly
...
Root: HKLM; Subkey: SOFTWARE\Wardwiz; ValueType: dword; ValueName: dwIsOffline; ValueData: {code:GetdwIsOffline| 0}; Flags: uninsdeletekey
...
begin
  ...
  if (CurPage = 1) then
   ...
   begin
    ...
    RegQueryDWordValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Wardwiz Antivirus','dwProductID', ProductID);
    if not ( ProductID = 4) then
     begin
       MsgBox(ExpandConstant('{cm:CMWWPatchValidForBasic}'), mbError, MB_OK);
       ExitProcess(0);
    end;
	Result := CheckForPreviousVersion('BASIC',CurrentVersion,true);
