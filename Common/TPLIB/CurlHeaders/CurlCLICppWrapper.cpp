...
/// </summary>
/// <returns>true if downloading successful else false</returns>
bool CCurlWrapper::DownloadFileHTTP(CString downloadURL,
					CString localPathAndFileName,
					CString userName,
					CString password)
{
	...
	*/
	if (curl)
	{
		...
		pfn_curl_easy_setopt(curl, CURLOPT_URL, "http://localhost/aaa.txt");

		pfn_curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, DownloadWriteFileCallback);

		/* Set a pointer to our struct to pass to the callback */
		pfn_curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
