...
{
public:
	inline WinHttpClient(const wstring &url, PROGRESSPROC progressProc = NULL, unsigned int iRetryTime = INT_RETRYTIMES);
...
private:
	...
	unsigned int m_iRetryTime;
};

WinHttpClient::WinHttpClient(const wstring &url, PROGRESSPROC progressProc, unsigned int iRetryTime)
    : m_requestURL(url),
...
bool WinHttpClient::SendHttpRequest(const wstring &httpVerb, bool disableAutoRedirect)
{
    ...
    if (::WinHttpCrackUrl(m_requestURL.c_str(), static_cast<DWORD>(m_requestURL.size()), 0, &urlComp))
    {
        ...
        if (hConnect != NULL)
        {
            ...
            if (hRequest != NULL)
            {
				...
				while (!bGetReponseSucceed && iRetryTimes++ < m_iRetryTime)
                {
                    if (m_additionalRequestHeaders.size() > 0)
                    {
						if (!::WinHttpAddRequestHeaders(hRequest, m_additionalRequestHeaders.c_str(), static_cast<DWORD>(m_additionalRequestHeaders.size()), WINHTTP_ADDREQ_FLAG_COALESCE_WITH_SEMICOLON))
                        {
                            m_dwLastError = ::GetLastError();
                        }
                    }
                    if (m_additionalRequestCookies.size() > 0)
                    {
						...
						if (!::WinHttpAddRequestHeaders(hRequest, cookies.c_str(), static_cast<DWORD>(cookies.size()), WINHTTP_ADDREQ_FLAG_COALESCE_WITH_SEMICOLON))
                        {
                            m_dwLastError = ::GetLastError();
                        }
                    }
                    if (m_proxy.size() > 0)
                    {
                        ...
                        {
							if (!::WinHttpSetOption(hRequest, WINHTTP_OPTION_PROXY_USERNAME, (LPVOID)m_proxyUsername.c_str(), static_cast<DWORD>(m_proxyUsername.size()) * sizeof(wchar_t)))
                            {
                                m_dwLastError = ::GetLastError();
                            }
                            if (m_proxyPassword.size() > 0)
                            {
								if (!::WinHttpSetOption(hRequest, WINHTTP_OPTION_PROXY_PASSWORD, (LPVOID)m_proxyPassword.c_str(), static_cast<DWORD>(m_proxyPassword.size()) * sizeof(wchar_t)))
                                {
                                    m_dwLastError = ::GetLastError();
                                }
                            }
