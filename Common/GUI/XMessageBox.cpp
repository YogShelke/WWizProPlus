	...
	m_bNarrow(pXMB->dwOptions & XMSGBOXPARAMS::Narrow)				//+++1.8
{
	...
	{
		//Issue no 1192 : Question mark icon is not display, Default icon is question mark
		LPTSTR lpIcon = (LPTSTR)IDI_QUESTION;
...
int CXDialogTemplate::Display()
{
	...
	*(WORD*)pdest = 0;		// use default window class		//+++1.5
	...
	if (hDlg)
	{
		...
		//Neha Gharge No disable owner by keeping second parameter true
		//If you want to disable we have keep second parameter as false;
		...
		while (!IsEnded())
		{
			if (::PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) 
			{
				...
				//Issue no 1011, When message box appear. If user minimize it from taskbar and maximize it
				//it is not getting restore. So we stop minimization.
