...
Author			: Ram Shelke
-----------------------------------------------------------------------------*/
bool CConnection::EstablishAdminConnection(void)
{
	try
	{
		...
		{
			OutputDebugString(L">>> Using username");
		...
		{
			OutputDebugString(L">>> without username");
		...
		if (dwRetVal != NO_ERROR)
		{
			...
			if (dwRetVal == ERROR_CANCELLED)
				strMsg.Format(L"RemoteIP: [%s] attempt to connect was canceled by the user!", m_csRemoteMachineName);
			else
				strMsg.Format(L"WNetAddConnection RemoteIP: [%s] Connection failed", m_csRemoteMachineName);
		...
		{
			CString strMsg;
			strMsg.Format(L"File RemoteIP: [%s] Connection failed", m_csRemoteMachineName);
			OutputDebugString(strMsg);
			return false;
		}
		objFile.Close();
		...
		{
			CString strMsg;
			strMsg.Format(L"Services [%s] Connection failed", m_csRemoteMachineName);
			OutputDebugString(strMsg);
			return false;
		}
...
-----------------------------------------------------------------------------*/
bool CConnection::CopyReqFilesToRemoteMachine(CString csMachineID)
{
	try
	{
		HKEY	hkey;
		...
		csStatusMsg.Format(L"RemoteIP: [%s] Installed path not found... Files copying error !", m_csRemoteMachineName);
...
bool CConnection::InstallClient(CString csMachineId)
{
	try
	{
		OutputDebugString(L"Copy Files!");
		if (!CopyReqFilesToRemoteMachine(csMachineId))
		{
			return false;
		}
		...
		CString csTemp;
		csTemp.Format(L"RemoteIP: [%s] Client service installed successfully!", m_csRemoteMachineName);
