...
//
void WriteEventEntry(PWSTR pszMessage, WORD wType)
{
	HANDLE hEventSource = NULL;
	LPCWSTR lpszStrings[2] = { NULL, NULL };
	TCHAR m_name[] =TEXT("WardwizClientAgent");
...
{
	//get here reqired details
	theApp.m_csProdRegKey = theApp.GetProductRegistryKey();
	theApp.m_csMachineID = theApp.GetMachineID();
...
{
	OutputDebugString(L">>> CServiceBase::OnStart");
	//get here required details
	theApp.m_csProdRegKey = theApp.GetProductRegistryKey();
	theApp.m_csMachineID = theApp.GetMachineID();
}
...
{
	OutputDebugString(L">>> CServiceBase::OnStop");
