...
Description				: source file that includes just the standard includes 
						  WrdWizRescueDisk.pch will be the pre-compiled header
...
#define LOG_FILE					_T("VIBORESCUDISK.LOG")
...
Date           : 11/02/2019
****************************************************************************/
void AddLogEntry(const TCHAR *sFormatString, const TCHAR *sEntry1, const TCHAR *sEntry2, bool isDateTime, DWORD dwLogLevel)
{
	try
	{
...
DWORD GetLoggingLevel4mRegistry()
{
	...
	{
		AddLogEntry(L"### Excpetion in GetLoggingLevel4mRegistry()", 0, 0, true, ZEROLEVEL);
	}
	return dwLogLevel;
}
...
DWORD Check4LogLevel(DWORD dwRegLogLevel, DWORD dwLogLevel)
{
	...
	{
		AddLogEntry(L"### Excpetion in Check4LogLevel()", 0, 0, true, ZEROLEVEL);
	}
	return dwRet;
}
...
CString GetWardWizPathFromRegistry()
{
	...
	{
		AddLogEntry(L"### Excpetion in GetWardwizPathFromRegistry()", 0, 0, true, ZEROLEVEL);
	}
	return L"";
}